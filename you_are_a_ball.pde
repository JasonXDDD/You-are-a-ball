...
void draw(){
  ...
  if(y>=405&&fall==false){
    ...
    strokeT = 60;
    strokeWT = 0;
    cRandom = int(random(360));
    shadowX = x+xv;
    if(rectLength>250){
      rectLength=rectLength*0.886;
    }else if(rectLength<=250&&rectLength>130)
      {rectLength = rectLength*0.907;
    }else if(rectLength<=130&&rectLength>80)
      {rectLength = rectLength*0.938; 
    }else if(rectLength<=80&&rectLength>=0)
      {rectLength = rectLength*0.965;
    }
    //println(score,frameT);
  }
  
  //more fast
  frameAddT = pow(2,-0.018*score+10.8)/pow(2,11);
  ...
  if(upT>=31 && (kP[UP] || kP[32])==false && showUp==true){
    pressUp = true;
    fill(d,100);
    rect(width/2-250,height/2-22.5,500,45);
    if(fDown==true){
      f -= 9;
    }
    if(f <= 50)fDown = false;
